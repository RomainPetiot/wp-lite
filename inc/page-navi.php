...
function wplite_page_navi($before = '', $after = '') {
	...
	if ($start_page >= 2 && $pages_to_show < $max_page) {
		$first_page_text = __( 'Première', 'wplite' );
		echo '<li><a href="'.get_pagenum_link().'" title="'.$first_page_text.'">'.$first_page_text.'</a></li>';
	}
	echo '<li>';
	previous_posts_link( __('Précédente', 'wplite') );
	echo '</li>';
	...
	echo '<li>';
	next_posts_link( __('Suivante', 'wplite'), 0 );
	echo '</li>';
	if ($end_page < $max_page) {
		$last_page_text = __( 'Dernière', 'wplite' );
